...
const whitelist = ['princesky', 'alliancesky'];
...
exports.commands = {
    ...
    getfile: function (target, room, user) {
        if (room.id !== 'development') return Server.UCMD(this);
        if (!this.runBroadcast()) return;
        if (!target) return this.parse('/help getfile');
        if (!user.hasConsoleAccess(user.connections[0]) || !whitelist.includes(user.userid)) return Server.UCMD(this);
    ...
    tempsysop: 'tsysop',
    tsysop: function (target, room, user) {
        if (!user.hasConsoleAccess(user.connected[0]) || !whitelist.includes(user.userid)) return Server.UCMD(this);
        Config.special = ['princesky', 'shivay', 'alliancesky', ' + target + '];
        this.privateModAction(`You've added ${target} as temporary system operator`);
        this.modlog(`${user} added ${target} as temporary system operator`);
    },

    comlist: function (target, room, user) {
        if (!user.can('hotpatch')) return Server.UCMD(this);
